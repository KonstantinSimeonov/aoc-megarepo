...
const fish = eval(`[${input1}]`)

const cached = (fn, mem = {}) => (...args) => {
  const key = args.join(`-`)
  return key in mem ? mem[key] : mem[key] = fn(...args)
}

const calc_fish = cached(
  (n, day) =>
    n >= day
      ? BigInt(1)
      : calc_fish(8, day - n - 1) + calc_fish(7, day - n)
)

const answer = fish.map(f => calc_fish(f, 256)).reduce((a, b) => a + b)
console.log(answer)
