do $$
declare
    input_text text = 'be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce';
    input_text1 text = 'fdceba bafdgc abeg afbdgec gbeacd abced bgc fcdge bg bedgc | bafdec cgefd gcebd ebcgd
...
gcedb fdcb aebgcd bf fcgabe cfbaedg fdbceg bfegd aedgf gbf | bfg cdgeb fdebg cebdg';
    stuff text[] = regexp_split_to_array(input_text, E'[\n\|]');
begin
    select array_agg(trim(x.x)) into stuff from (
        select x, row_number() over () as i from unnest(stuff) as x
    ) as x where i % 2 = 0;
    stuff := string_to_array(
        (select string_agg(x, ' ') from unnest(stuff) as x),
        ' '
    );
    raise notice '%', stuff;
    raise notice '%', (select count(x) from unnest(stuff) as x where length(x) in (2, 4, 3, 7));
end $$;
