...
fn checksum(disk: &[usize]) -> usize {
  disk.iter().zip(0..).filter(|&(bl, _)| *bl != FREE).map(
    |(id, i)| id * i
  ).sum()
}
...
fn solve(input: &str) -> (usize, usize) {
  let mut blks: Vec<usize> = input.trim().bytes().zip(0..).flat_map(|(c, i)| {
  ...
  let blks2: Vec<(usize, usize)> = input.trim().bytes().map(|b| (b - b'0') as usize).zip(0..).collect();
...
fn p(blocks: &[usize]) {
  for &b in blocks {
    print!("{}", if b == FREE { ".".to_owned() } else { b.to_string() })
  }
